			...
			while((zeichen=getc(fp1)) != EOF)
			{
				if(zeichen=='#')
				{
					...
					if(crosscnt==1)
					{
						...
						fprintf(fp2, "%s", timebuf2);
					}
					if(crosscnt==2) fprintf(fp2, "%s", timebuf);
					if(crosscnt==3) fprintf(fp2, "%hu", z2);
					if(crosscnt==4) fprintf(fp2, "%hu", z3);
					if(crosscnt==5) fprintf(fp2, "%hu", total_qso);
					if(crosscnt==6) fprintf(fp2, "%hu", max_qso);
